...
def extract_lab_test_data(ocr_text: str) -> List[Dict[str, Any]]:
    """Extract lab test data from OCR text and return as dictionaries instead of LabTest objects."""
    ocr_text = re.sub(r'\s+', ' ', ocr_text)  # Replace multiple whitespaces/newlines with single space
    ocr_text = ocr_text.upper()               # Convert to uppercase for consistency
    print("Preprocessed OCR Text:", ocr_text) 
    lab_tests = []
    
    # Extract test data using predefined regex patterns
    ...
    for test in tests:
        ...
        test_unit = test['test_unit'] or "N/A"  # Default to "N/A" if unit is missing
        bio_reference_range = test['bio_reference_range']
        
        # Parse the bio reference range (min_value - max_value)
        try:
            ref_parts = re.findall(r"[\d.-]+", bio_reference_range)
            if len(ref_parts) >= 2:
                ref_min, ref_max = map(float, ref_parts[:2])
                # Check if the test value is out of range (if reference range is available)
                try:
        ...
        # Create lab test entry as a dictionary instead of LabTest object
        lab_tests.append({
            "test_name": test_name,
...
def extract_with_patterns(text: str) -> List[Dict]:
    """Extract tests using predefined regex patterns."""
    tests = []

    # Use the predefined patterns for extracting test data
    for pattern in TEST_PATTERNS:
        ...
        for match in matches:
            ...
            # Use reference ranges from predefined config or defaults
