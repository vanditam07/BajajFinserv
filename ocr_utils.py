...
def process_image(image_bytes: bytes) -> np.ndarray:
    """Preprocess image bytes for optimal OCR performance."""
    try:
...
def extract_text(image_np: np.ndarray) -> str:
    """Extract text from processed image using EasyOCR."""
    try:
        # Initialize EasyOCR reader for the specified language
        reader = easyocr.Reader([OCR_CONFIG.get('lang', 'en')])  # Default to English if no language is specified
        
        # EasyOCR expects a list of images, so wrap the image in a list
